...
For the first research-preview release, `review-gated` MathType output is not an automated deliverable claim. It means Word export succeeded and traceable visual evidence exists, but a human reviewer must inspect layout drift and formula risk before production use.
...
Do not describe current MathType output as lossless, pixel-identical, or production-ready without review. Current evidence supports a research-preview manual-review workflow: complete conversion and replacement counts, Word export, page-count checks, unmatched-page checks, visual changed-ratio metrics, and human inspection.

Public marker fixtures are detector/routing fixtures only. They are useful for source discovery and dry-run planning, but they are not proof that the live MTEF-to-OMML conversion path works. Real binary MTEF OLE payloads used for local control runs must stay out of the public repository unless a separate fixture task records the upstream source, SHA-256, license, NOTICE attribution, and artifact hygiene.
...
Do not publish private or copyrighted `.docx` files as test fixtures. Prefer small synthetic fixtures that demonstrate one formula pattern at a time.
