...
AxMath support remains public export-gated for conversion. The public package includes a no-payload contract validator and bring-your-own local inspection commands: `inspect-axmath-local` for user-supplied local DOCX/OLE/Contents inputs and `inspect-axmath-contents` for a raw `Contents` byte stream. These commands check only the bounded 13 + 20n `Contents` byte-grid shape and do not retain source bytes in their reports. The package does not vendor private AxMath source files, binary payload material, formula bodies, generated canonical bodies, or public real-payload fixtures, and it does not emit canonical MathML. A private controlled-v2 AxMath acceptance run is represented publicly only as no-payload contract metadata: aggregate counts, result classes, diagnostics, artifact boundaries, and false public claim boundaries. Do not treat the local structural scanner or controlled acceptance contract as semantic native parsing, universal AxMath support, public fixture eligibility, conversion support, or automated deliverability.

Equation Editor 3.0 support is implemented only for the current MTEF v2/v3 source-core slice. It can emit canonical MathML with provenance for supported DOCX OLE embeddings and legacy `.doc` ObjectPool `Equation Native` streams, but it does not claim universal historical Equation Editor 3.0 support or Word/DOCX deliverability. Public Equation3 fixtures are minimized native-stream controls; full upstream `.doc` files are not vendored.
